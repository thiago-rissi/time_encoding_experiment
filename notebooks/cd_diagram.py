...
def draw_cd_diagram(metric: str, df_perf=None, alpha=0.05, title=None, labels=False):
    """
...
def wilcoxon_holm(metric: str, alpha=0.05, df_perf=None):
    """
    ...
    for i in range(m - 1):
        ...
        perf_1 = np.array(
            df_perf.loc[df_perf["classifier_name"] == classifier_1][metric],
            dtype=np.float64,
        )
        for j in range(i + 1, m):
            ...
            # get the performance of classifier one
            perf_2 = np.array(
                df_perf.loc[df_perf["classifier_name"] == classifier_2][metric],
                dtype=np.float64,
            )
